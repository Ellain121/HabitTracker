...
RelativeResultGraph::RelativeResultGraph(QWidget *parent)
    : QWidget{parent}, ui{new Ui::RelativeResultGraph} {}

RelativeResultGraph::~RelativeResultGraph() { delete ui; }
...
RelativeResultGraph::LocalSettings RelativeResultGraph::getLocalSettings() {
  int nDays = toInt(Settings::instance().getSetting(mHabit->getName(),
                                                    "n_days_spin_box_rrg"));
  int tickCBIndx = toInt(Settings::instance().getSetting(
      mHabit->getName(), "x_tick_combo_box_indx_rrg"));
  qDebug() << tickCBIndx << " " << ui->xTickComboBox->count();
  // TimePeriod tp = toTP(ui->xTickComboBox->itemText(tickCBIndx));
  TimePeriod tp = static_cast<TimePeriod>(0);
...
bool RelativeResultGraph::addGraph()
{
  ...
  if (timePeriod != TimePeriod::N_Days) {
    ...
    for (auto it = habitData.begin(); it != habitData.end(); ++it) {
      ...
      int nextDayPeriodNumber =
          getTimePeriodNumber(date.addDays(1), timePeriod);
  ...
  JKQTPXYGraphLabels *graphLabels = new JKQTPXYGraphLabels{
      JKQTPXYGraphLabels::XValueLabel, ui->relativeResultPlot};
...
void RelativeResultGraph::setPlotterSettings() {
  ...
  ui->relativeResultPlot->getXAxis()->setTickLabelColor(
...
void RelativeResultGraph::setPlotterAbsoluteX() {
  ...
  ui->relativeResultPlot->setAbsoluteX(
      startDate.toMSecsSinceEpoch() - periodInMSec,
...
void RelativeResultGraph::setPlotterZoom() {
  ...
  ui->relativeResultPlot->setXY(stDate.toMSecsSinceEpoch() - TPMsec,
                                curDate.toMSecsSinceEpoch() + TPMsec, -145,
                                345);
  //	ui->relativeResultPlot->setXY(newestDate.toMSecsSinceEpoch() -
  //MONTH_IN_MSEC, 		static_cast<double>(newestDate.toMSecsSinceEpoch()), -145,
  //345);
}

void RelativeResultGraph::initGraphInfoWidgets() { initXTickGroupBox(); }
...
void RelativeResultGraph::initXTickGroupBox() {
  ...
  connect(ui->xTickComboBox, &QComboBox::currentIndexChanged, this,
          [this](int currentIndx) {
            bool isNDays = (ui->xTickComboBox->itemText(currentIndx) ==
                            toQString(TP::N_Days));
            ui->nDaysSpinBox->setEnabled(isNDays);
          });

  connect(ui->tickApplyButton, &QPushButton::clicked, this, [this]() {
    ...
    Settings::instance().setSetting(mHabit->getName(),
                                    "x_tick_combo_box_indx_rrg",
                                    ui->xTickComboBox->currentIndex());
    Settings::instance().setSetting(mHabit->getName(), "n_days_spin_box_rrg",
                                    ui->nDaysSpinBox->value());
  });
  /*** [END: Connections] ****/
